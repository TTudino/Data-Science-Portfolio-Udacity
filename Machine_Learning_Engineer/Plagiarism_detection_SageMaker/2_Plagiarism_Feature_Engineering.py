...
def numerical_dataframe(csv_file='data/file_information.csv'):
    ...
    original_data['Class'] = original_data['Category'].map({'non':0,'heavy':1,'light':1 , 'cut':1, 'orig':-1})
    original_data['Category'] = original_data['Category'].map({'non':0,'heavy':1,'light':2 , 'cut':3, 'orig':-1})
...
def calculate_containment(df, n, answer_filename):
    ...
    data_df = df.loc[complete_df['File'] == answer_filename]

    task = data_df['Task'].values[0]
    
    # answer text
    a_text = data_df['Text'].values[0]
    # source text
    ...
    numerator = np.minimum(ngrams[0],ngrams[1]).sum()

    denominator = ngrams[0].sum()
...
# **Answer:**
# The theory behind the calculation of the containment features states that plagiarism can be identify by compairing single letters (words) or group of letters (words) between two or more texts. This comparison, also known as pairwise comparison, should return a similar estimate when the entire dataset or a random subsample of it is considered. As a consequence, the containment estimate is indipendent from the division between train and test datasets and it could be done a priori.
# 
...
def train_test_data(complete_df, features_df, selected_features):
    ...
    df_original = pd.concat([complete_df,features_df], axis =1)
    
    df_train = df_original[df_original['Datatype']=='train']
    df_test = df_original[df_original['Datatype']=='test']
...
# **Answer:** 
# In order to create a reliable model, we should avoid the inclusion of fields that are correlated. For istance, the correlation index between c_2 and c_3 is 0.99. This suggests that the two fields are highly correlated and hence only one of them should be included in the model. Similarly, only one between c_3 and c_6 should be included, being their correlation coefficient equal to 0.98.
# 
...
def make_csv(x, y, filename, data_dir):
    ...
    # Prepare the dataframe and save it as a csv
    dataframe_to_save = pd.concat([pd.DataFrame(y), pd.DataFrame(x)], axis=1, sort=False)
    store_dir = data_dir + "/" + filename
    dataframe_to_save.to_csv(store_dir, index=False, header=False)
